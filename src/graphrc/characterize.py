...
def calculate_atom_displacement(
    atom_idx: int, frame_ts: Dict[str, Any], frames_displaced: List[Dict[str, Any]]
) -> float:
    """
...
def identify_moving_group(
    hub_atom: int, frame_ts: Dict[str, Any], frames_displaced: List[Dict[str, Any]], connectivity: Dict[int, set]
) -> Tuple[int, float, str]:
    """
    ...
    logger.debug(f"Most mobile group: {description} at position {neighbor_idx} (displacement: {max_disp:.3f} Å)")
...
def classify_rotation_type(
    dihedral: Tuple[int, int, int, int], frame_ts: Dict[str, Any], dihedral_change: float, connectivity: Dict[int, set]
) -> Dict[str, Any]:
    """
...
def characterize_vib_mode(
    internal_changes: Dict[str, Any], frames: List[Dict[str, Any]], ts_frame_idx: int = 0
) -> Dict[str, Any]:
    """
    ...
    if has_dihedrals:
        ...
        if inversion_result:
            ...
            if group_description == "unknown" or max_disp < 0.01:
                ...
                logger.info(f"Characterized as {element} inversion (hub atom {hub_atom}, moving {group_description})")
